...
var formRules = []form.Rule{
	{
		...
		Handler: func(ctx types.Context, values map[string]interface{}) types.MsgPayload {
			for key, value := range values {
				...
				case "zone_id":
					err := store.Chatbot.ConfigSet(ctx.AsUser, ctx.Original, ZoneIdKey, map[string]interface{}{
						"value": value.(string),
					})
					if err != nil {
						logs.Err.Println(err)
				...
				case "account_id":
					err := store.Chatbot.ConfigSet(ctx.AsUser, ctx.Original, AccountIdKey, map[string]interface{}{
						"value": value.(string),
					})
					if err != nil {
						logs.Err.Println(err)
