...
func newCounterRecord(db *gorm.DB, opts ...gen.DOOption) counterRecord {
	...
	_counterRecord.CounterID = field.NewInt32(tableName, "counter_id")
...
func (c *counterRecord) updateTableName(table string) *counterRecord {
	...
	c.CounterID = field.NewInt32(table, "counter_id")
...
func (c counterRecordDo) GetByID(id int) (result model.CounterRecord, err error) {
	...
	generateSQL.WriteString("SELECT * FROM chatbot_counter_records WHERE id=? ")
...
func (c counterRecordDo) Exists(subquery interface{ UnderlyingDB() *gorm.DB }) *counterRecordDo {
